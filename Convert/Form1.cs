...
namespace Convert
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            ColorSet(Convert.Properties.Settings.Default.Color, false);
            ...
            dataGridView3.Columns[7].DefaultCellStyle.Font = new Font("Cabin", dataGridView3.Font.Size);

            if (!File.Exists(path))
            {
                MessageBox.Show("Export File doesn't Exist");
            }
            else
            {
                ...
                if (categoryload == true)
                {
                    for (int d = 0; d < cats.Length; d++)
                    {
                        ...
                        {
                            if (cat.Contains(Purchases.Rows[f][2].ToString()))
                            {
        ...
        {
            ColorSet(e.ClickedItem.Text.ToString(), true);
            Properties.Settings.Default.Color = e.ClickedItem.Text.ToString();
            Properties.Settings.Default.Save();
        }

        public void ColorSet(string sent, Boolean manualclick)
        {
            if (sent == "White")
            {
                back = Color.White;
            ...
            else if (sent == "Custom")
            {
                ...
                back = Properties.Settings.Default.back;
                front = Properties.Settings.Default.front;
                front2 = Properties.Settings.Default.front2;
                text = Properties.Settings.Default.text;
                glow = Properties.Settings.Default.glow;
            }
            Colors();
        }
        ...
        void cms_Opening(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            fruitContextMenuStrip.Items.Add("Custom");
        ...
        {
            List<string> cats = new List<string> (File.ReadAllLines(root + @"\categories.save").ToArray());
        ...
        private void CategoryRefresh(Boolean changed, int ar)
        {
            ...
            //totals categories in bottom
            {
                ...
                {
                    dataGridView6.Rows[0].Cells[7].Value = "+" + negpos;
                    dataGridView6.Rows[0].Cells[7].Style.ForeColor = ColorTranslator.FromHtml("#52e3c2");
                }
                if (negpos < 0)
                {
                    dataGridView6.Rows[0].Cells[7].Value = negpos;
                    dataGridView6.Rows[0].Cells[7].Style.ForeColor = ColorTranslator.FromHtml("#ff4495");
                }
            }
            ...
            for (int r = 0; r < dataGridView3.Rows.Count; r++)
            {
                ...
                {
                    dataGridView3.Rows[r].Cells[7].Style.ForeColor = ColorTranslator.FromHtml("#52e3c2");
                }
                if (double.Parse(dataGridView3.Rows[r].Cells[7].Value.ToString()) < 0)
                {
                    dataGridView3.Rows[r].Cells[7].Style.ForeColor = ColorTranslator.FromHtml("#ff4495");
                }
            }
            this.Refresh();
        }
        ...
        private void DataGridView3_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (numberfix == false)
            {
                if (e.ColumnIndex == 6)
                {
                    ...
                    if (!long.TryParse(dataGridView3[6, e.RowIndex].Value.ToString(), out result))
                    {
                        numberfix = true;
                        dataGridView3.Rows[e.RowIndex].Cells[6].Value = startbudget;
        ...
        {
            startbudget = double.Parse(dataGridView3[6, e.RowIndex].Value.ToString(), System.Globalization.CultureInfo.InvariantCulture);
        }
